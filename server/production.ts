// server/production.ts - Production server that starts HTTP server
...
(async () => {
  ...
  app.use((err: any, _req: Request, res: Response, _next: NextFunction) => {
    console.error('❌ Production Request Error:', {
      message: err.message,
      stack: err.stack,
      url: _req.url,
      method: _req.method
  ...
  if (fs.existsSync(distPath)) {
    ...
    console.warn(`⚠️  Build directory not found: ${distPath}. Run 'npm run build' first.`);
  }
  ...
  server.listen(port, '0.0.0.0', () => {
    console.log('\n=== RiderPro Delivery Management System (Production) ===');
    ...
    console.log(`🗄️  Database: PostgreSQL with Supabase`);
