...
// Only create backup pool if explicitly configured (not required for basic operation)
const backupDbUrl = process.env.BACKUP_DATABASE_URL;
const hasBackupDb = !!backupDbUrl && backupDbUrl.trim() !== '';
const isBackupSslDisabled = backupDbUrl?.includes('sslmode=disable') ?? false;

const backupDbConfig = hasBackupDb ? {
...
// Export backup pool (only if configured and in dev/alpha environments)
export const backupPool = (hasBackupDb && (isDevelopment || isLocalhost) && backupDbConfig) 
  ? new Pool(backupDbConfig) 
  : null;
...
export const initTables = async () => {
  ...
  try {
    ...
    // Note: CREATE TABLE IF NOT EXISTS doesn't need a transaction
    // and using transactions can cause issues with index creation
    ...
    // Add access_token column to user_sessions if it doesn't exist (for existing Supabase tables)
    try {
      await client.query(`ALTER TABLE user_sessions ADD COLUMN IF NOT EXISTS access_token TEXT`);
    } catch (error) {
      log.warn('Failed to add access_token column to user_sessions (might already exist)', error);
    }

    // Make session_token nullable if it exists (for Supabase auth compatibility)
    try {
      await client.query(`ALTER TABLE user_sessions ALTER COLUMN session_token DROP NOT NULL`);
    } catch (error) {
      // Column might not exist, which is fine
      log.dev('session_token column not found or already nullable');
    }

    // Add missing columns if they don't exist
    try {
      await client.query(`ALTER TABLE shipments ADD COLUMN IF NOT EXISTS acknowledgment_captured_by VARCHAR(255)`);
    } catch (error) {
      log.warn('Failed to add acknowledgment_captured_by column (might already exist)', error);
    }

    // Add missing columns to users table if they don't exist (for existing Supabase auth tables)
    const userColumns = [
      'employee_id VARCHAR(255)',
      'full_name VARCHAR(255)',
      'access_token TEXT',
    ...
    for (const columnDef of userColumns) {
      ...
      try {
        ...
        log.dev(`Added column ${columnName} to users table`);
      } catch (error) {
        log.warn(`Failed to add ${columnName} column (might already exist)`, error);
      }
    }

    // Make password_hash nullable for external auth (POPS API)
    try {
      await client.query(`ALTER TABLE users ALTER COLUMN password_hash DROP NOT NULL`);
    } catch (error) {
      // Column might not exist or already nullable, which is fine
      log.dev('password_hash column not found or already nullable');
    }
    ...
    if (!tableCheck.rows[0].exists) {
      throw new Error('shipments table was not created');
    }
    ...
    const indexQueries = [
      ...
      // Skip sync_status index for now - create it separately after table is confirmed
      ...
      'CREATE INDEX IF NOT EXISTS idx_shipments_location ON shipments(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL',
    ...
    for (const query of indexQueries) {
      try {
        ...
        log.warn(`Failed to create index (might already exist): ${query}`, error);
      }
    }
...
export const syncToBackup = async () => {
  ...
  try {
    ...
    // Check if backup database has the shipments table
    ...
    try {
      ...
      if (!tableCheck.rows[0]?.exists) {
        log.warn('Backup database tables not initialized yet, skipping sync');
        return;
      }
    } finally {
    ...
    const backupClient2 = await backupPool.connect();
    ...
    try {
      ...
      await backupClient2.query('DELETE FROM shipments WHERE "createdAt" < $1', [threeDaysAgo]);

      // Insert recent data
      for (const shipment of recentShipments.rows) {
        await backupClient2.query(
